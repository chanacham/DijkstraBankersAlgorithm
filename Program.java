...
public class Program {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < NUM_PROCS; i++){
            processes.add(new Process(MAX_PROC_RESOURCES - rand.nextInt(3))); // Initialize to a new Proc, with some small range for its max
        }
        ...
        for (int i = 0; i < ITERATIONS; i++) {
            ...
            for (int j = 0; j < processes.size(); j++) {
                ...
                if (currRequest == 0) {
                    System.out.println("Continue to next request");
                    continue;
                } else if (currRequest < 0) {
                    System.out.println("Process finished, releasing resources");
                    ...
                    boolean canGrant = checkCanGrant(processes, currRequest, j);
                    if (canGrant) {
    ...
    private static boolean checkCanGrant(ArrayList<Process> processes, int currRequest, int currProcess) {
        ArrayList<Process> copyProcesses = new ArrayList<>(processes);

        int available = TOTAL_RESOURCES - totalHeldResources - currRequest;
        copyProcesses.get(currProcess).addResources(currRequest);
        ...
        while (!copyProcesses.isEmpty() && found) {
            ...
            for (int ix = 0; ix < copyProcesses.size(); ix++) {
                ...
                if ((curr.getMaxResources() - curr.getHeldResources()) <= available) {
                    available -= curr.getHeldResources(); // grant request
                    indexHolder.add(ix);
                    found = true;
                }
            }
        ...
        //processes.get(currProcess).setHeldResources(processes.get(currProcess).getHeldResources()-currRequest);
        return found;
    }
}